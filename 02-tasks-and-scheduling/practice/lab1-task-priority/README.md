...
Task ที่ทำงานมากที่สุดในแง่ของ การบริโภคเวลา CPU รวม (ถ้าโหลดของระบบต่ำ) คือ Idle Task (Priority 0/ต่ำที่สุด)

เหตุผล: Idle Task ถูกเรียกใช้เมื่อ Scheduler ไม่มี Task อื่นใดที่พร้อมรัน เพื่อป้องกันไม่ให้ CPU ว่าง (Halt) และใช้ในการวัด CPU Load หรือเข้าสู่โหมดประหยัดพลังงาน

2. เกิด Priority Inversion หรือไม่? จะแก้ไขได้อย่างไร?
เกิดขึ้นได้ (โดยเฉพาะเมื่อใช้ Mutex) เมื่อ Task ลำดับสูงถูกบล็อกเพราะต้องรอทรัพยากรที่ Task ลำดับต่ำกว่าถืออยู่ และ Task ลำดับกลางเข้ามารันขัดจังหวะ Task ลำดับต่ำกว่านั้น

แก้ไขโดย ใช้กลไก Priority Inheritance (การสืบทอด Priority) ซึ่งทำให้ Task ลำดับต่ำจะ ยกระดับ Priority ตัวเองชั่วคราวให้เท่ากับ Task ลำดับสูงที่กำลังรอทรัพยากรนั้นอยู่

3. Tasks ที่มี priority เดียวกันทำงานอย่างไร?
ทำงานแบบ Time Slicing (Round-Robin)

Scheduler จะให้ เวลาทำงาน (Time Quantum) ที่เท่ากันแก่ Task ที่พร้อมรัน เมื่อเวลานั้นหมดลง Task จะถูกสลับออก (Preempted) ไปยัง Task ถัดไปในลำดับเดียวกัน

4. การเปลี่ยน Priority แบบ dynamic ส่งผลอย่างไร?
ส่งผลให้ Task สามารถ ยกระดับหรือลดระดับความสำคัญ ของตัวเองหรือ Task อื่น แบบเรียลไทม์ (ผ่าน vTaskPrioritySet())

มีประโยชน์ในการจัดการโหลดงานฉุกเฉิน หรือใช้เป็นกลไกในการแก้ปัญหา Priority Inversion (ผ่าน Priority Inheritance)

5. CPU utilization ของแต่ละ priority เป็นอย่างไร?
ไม่ได้กระจายเท่ากัน

CPU จะถูกใช้โดย Task ลำดับสูงที่สุดที่พร้อมรัน เสมอ Task ลำดับต่ำจะทำงานก็ต่อเมื่อ Task ลำดับสูงกว่าทั้งหมด ถูกบล็อก (Blocked) หรือ หยุดทำงานชั่วคราว (Yield) แล้วเท่านั้น การใช้ CPU จึงถูกกำหนดโดยความต้องการและพฤติกรรมของ Task นั้น ๆ
