...
ขึ้นอยู่กับประเภทของ Semaphore:

Binary Semaphore: หากมีสถานะเป็น Available อยู่แล้ว การ give ซ้ำจะ ไม่เกิดผลใด ๆ เพราะจำนวนนับสูงสุดของมันคือ 1

Counting Semaphore: จำนวนนับของ Semaphore จะ เพิ่มขึ้น ทุกครั้งที่ give จนกว่าจะถึงจำนวนนับสูงสุด (Maximum Count) ที่ถูกกำหนดไว้ตอนสร้าง

2. ISR สามารถใช้ xSemaphoreGive หรือต้องใช้ xSemaphoreGiveFromISR? ⚙️
ต้องใช้ xSemaphoreGiveFromISR เสมอ

เหตุผล: ฟังก์ชัน FromISR ถูกออกแบบมาเพื่อความ ปลอดภัยในการทำงานใน ISR โดยจะจัดการกับการสลับบริบท (Context Switch) ให้อยู่ในรูปแบบที่ถูกต้องตาม FreeRTOS ทำให้ Task ลำดับสูงที่ถูกปลุกให้ตื่นสามารถเริ่มรันได้ทันทีที่ ISR สิ้นสุดการทำงาน

3. Binary Semaphore แตกต่างจาก Queue อย่างไร? ↔️
ความแตกต่างอยู่ที่ วัตถุประสงค์หลัก:

Binary Semaphore: ใช้เพื่อ การซิงโครไนซ์ (Synchronization) หรือ การควบคุมการเข้าถึง (Mutual Exclusion) โดยไม่มีการส่งข้อมูลใด ๆ มีแค่สถานะ 0 หรือ 1

Queue: ใช้เพื่อ การส่งผ่านข้อมูล (Message Passing) โดยมีข้อมูล (Payload) ถูกคัดลอกระหว่าง Task ผู้ส่งและผู้รับเสมอ
## 📋 สรุปผลการทดลอง
