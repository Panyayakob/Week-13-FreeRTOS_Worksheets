...
| 3 (Changed Priority) | 48 | 20 | 0 | 96 | |

### คำถามสำหรับการทดลอง
1. เมื่อไม่ใช้ Mutex จะเกิด data corruption หรือไม่? 💥
เกิดได้แน่นอน เมื่อ Task หลายตัวเข้าถึงและแก้ไขข้อมูลที่ใช้ร่วมกันพร้อมกัน (เรียกว่า Critical Section) การทำงานของ Task ที่กำลังเขียนข้อมูลอาจถูก ขัดจังหวะ (Preempted) โดย Task ลำดับสูงกว่า ทำให้ข้อมูลไม่สอดคล้องหรือเสียหาย ซึ่งปัญหานี้เรียกว่า Race Condition

2. Priority Inheritance ทำงานอย่างไร? ⬆️
Priority Inheritance เป็นกลไกป้องกัน Priority Inversion เมื่อ Task ลำดับสูงถูกบล็อกรอ Mutex ที่ Task ลำดับต่ำถืออยู่ ระบบจะ ยกระดับ Priority ของ Task ลำดับต่ำชั่วคราวให้เท่ากับ Task ลำดับสูงที่กำลังรอ เพื่อให้ Task ลำดับต่ำทำงานส่วน Critical Section เสร็จสิ้นอย่างรวดเร็วและปล่อย Mutex ก่อนที่จะถูก Task ลำดับกลางขัดขวาง

3. Task priority มีผลต่อการเข้าถึง shared resource อย่างไร? 🥇
ถ้าไม่ใช้ Mutex: Task Priority มีผลทำให้ Task ลำดับสูงสามารถ ขัดจังหวะ (Preempt) การทำงานของ Task ลำดับต่ำที่กำลังใช้ทรัพยากรนั้นอยู่ทันที ซึ่งนำไปสู่ Data Corruption

ถ้าใช้ Mutex: Priority มีผลต่อ ลำดับการได้ Mutex หาก Task หลายตัวรอ Mutex พร้อมกัน Task ที่มี Priority สูงสุดจะได้รับ Mutex ก่อน และเป็นปัจจัยสำคัญที่ทำให้เกิดความเสี่ยงของ Priority Inversion ได้ครับ
