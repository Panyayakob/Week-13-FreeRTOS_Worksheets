...
เมื่อจำนวน Producers (Task ที่ต้องการใช้ทรัพยากร) มีมากกว่าจำนวน Resources (ทรัพยากรที่มีอยู่จริง) Task ส่วนเกินจะถูก บล็อก (Blocked) และเข้าสู่สถานะ รอคอย (Waiting)

ในทางปฏิบัติ: Task ที่ถูกบล็อกจะรอจนกว่า Task อื่นจะใช้งานทรัพยากรเสร็จและ คืน (Release) ทรัพยากรนั้นกลับมา (เช่น การ give Semaphore)

ถ้าไม่ใช้กลไกควบคุม (Semaphore): จะเกิด Race Condition และ Data Corruption ของทรัพยากรที่ใช้ร่วมกัน

2. Load Generator มีผลต่อ Success Rate อย่างไร? 📉
Load Generator (Task ที่สร้างงาน) มีผลโดยตรงต่อ Success Rate ของระบบ:

ถ้า Load ต่ำ: Success Rate จะ สูง Task ส่วนใหญ่จะสามารถเข้าถึงทรัพยากรได้ทันที

ถ้า Load สูง: Success Rate จะ ลดลง เนื่องจาก Task จะต้องใช้เวลาส่วนใหญ่อยู่ในสถานะ Blocked เพื่อรอทรัพยากรหรือรอพื้นที่ใน Queue (Buffer) ซึ่งนำไปสู่การ Time Out หรือการประมวลผลงานไม่ทันตามกำหนดเวลา (Deadline)

3. Counting Semaphore จัดการ Resource Pool อย่างไร? 🏊
Counting Semaphore ใช้เพื่อจัดการ Resource Pool (ทรัพยากรที่มีหลายชุด) ที่สามารถใช้พร้อมกันได้:

จำนวนนับ (Count) เริ่มต้น: ถูกตั้งให้เท่ากับ จำนวนทรัพยากรที่มีอยู่ ใน Pool

การเข้าใช้: เมื่อ Task ต้องการใช้ทรัพยากร มันจะทำการ take Semaphore จำนวนนับจะลดลง 1

การคืน: เมื่อ Task ใช้ทรัพยากรเสร็จ มันจะทำการ give Semaphore จำนวนนับจะเพิ่มขึ้น 1

การบล็อก: หาก Task พยายาม take Semaphore เมื่อจำนวนนับเป็น ศูนย์ Task นั้นจะ ถูกบล็อก จนกว่าจะมีทรัพยากรถูกคืนกลับมา
